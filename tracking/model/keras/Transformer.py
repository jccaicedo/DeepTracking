...
class Transformer(Layer):
    ...
    def call(self, X, mask=None):
        if type(X) is not list or len(X) <= 1:
            raise Exception('Transfoemer must be called on a list of tensors '
    ...
    def transform(theta, input, downsample_factor):
        ...
        grid = Transformer.meshgrid(out_height, out_width)
        ...
        input_transformed = Transformer.interpolate(
    ...
    def meshgrid(height, width):
        ...
        x_t = T.dot(T.ones((height, 1)),
                    Transformer.linspace(-1.0, 1.0, width).dimshuffle('x', 0))
        y_t = T.dot(Transformer.linspace(-1.0, 1.0, height).dimshuffle(0, 'x'),
                    T.ones((1, width)))
    ...
    def linspace(start, stop, num):
        ...
        start = T.cast(start, K.floatx())
        stop = T.cast(stop, K.floatx())
        num = T.cast(num, K.floatx())
        step = (stop-start)/(num-1)
        return T.arange(num, dtype=K.floatx()) * step + start
